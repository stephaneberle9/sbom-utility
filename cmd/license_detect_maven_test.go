...
func innerTestFindLicenseInPom(t *testing.T, group string, name string, version string, expectedLicense string, expectedLicenseUrl string) {
	innerTestFindLicensesInPom(t, group, name, version, []string{expectedLicense}, []string{expectedLicenseUrl})
}

func innerTestFindLicensesInPom(t *testing.T, group string, name string, version string, expectedLicenses []string, expectedLicenseUrls []string) {
	...
	if len(pomLicenses)/2 > len(expectedLicenses) {
		t.Errorf("to many licenses found in POM of component `%v`\n", cdxComponent)
		return
	}

	foundLicenses := make([]string, len(pomLicenses)/2)
	foundLicenseUrls := make([]string, len(pomLicenses)/2)
	for i, pomLicense := range pomLicenses {
		if i%2 == 0 {
			foundLicenses[i/2] = pomLicense
		} else {
			foundLicenseUrls[(i-1)/2] = pomLicense
		}
	}
...
func TestFindLicenseInPom(t *testing.T) {
	...
	innerTestFindLicenseInPom(t, GROUP, NAME, VERSION, EXPECTED_LICENSE, EXPECTED_LICENSE_URL)
...
func TestFindLicensesInPom(t *testing.T) {
	...
	innerTestFindLicensesInPom(t, GROUP, NAME, VERSION, EXPECTED_LICENSES, EXPECTED_LICENSE_URLS)
}
